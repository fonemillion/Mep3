...
def toData(filename):
    ...
    for node in zDict:
        if node in lvs:
            ...
            x.append(getLeafFeat(treeSet,node,nodeDict) + [pickble])
            if node in bestPick:
                y.append(1)
            #elif leaf in nonTemp:
            #    y.append(1)
            else:
                y.append(0)
        else:
    ...
    edge_attr = []
    edge_index = []
    leafpairs = [[n,m] for n in range(len(zDict)) for m in range(len(zDict)) if n != m and zDict[n] in lvs and zDict[m] in lvs]
    for pair in leafpairs:
        #leaf1 = pair[0]-1
        #leaf2 = pair[1]-1
        edge_attr.append(getEdgeFeat(nodeDict, treeSet, (zDict[pair[0]],zDict[pair[1]])))
        edge_index.append(pair) # -1 because no root
    ...
    z4Edges = []
    z5Edges = []
    for nodeA in range(len(zDict)):
        for nodeB in range(len(zDict)):
            if nodeA == nodeB:
                continue
            if zDict[nodeA] in nodeDict[zDict[nodeB]]:
                z2Edges.append((nodeA,nodeB))
                z3Edges.append((nodeB,nodeA))
            if len(set(nodeDict[zDict[nodeA]]) ^ set(nodeDict[zDict[nodeB]])) == 1 or len(set(nodeDict[zDict[nodeA]]) ^ set(nodeDict[zDict[nodeB]])) == 2 :
                try:
                    nx.shortest_path_length(treeSet[tree], source=zDict[nodeA], target=zDict[nodeB])
                except:
                    try:
                        nx.shortest_path_length(treeSet[tree], source=zDict[nodeB], target=zDict[nodeA])
                    except:
                        z4Edges.append((nodeA,nodeB))
                        z4Edges.append((nodeB,nodeA))
            if len(set(nodeDict[zDict[nodeA]]) ^ set(nodeDict[zDict[nodeB]])) == 3 or len(set(nodeDict[zDict[nodeA]]) ^ set(nodeDict[zDict[nodeB]])) == 4:
                try:
                    nx.shortest_path_length(treeSet[tree], source=zDict[nodeA], target=zDict[nodeB])
                except:
                    try:
                        ...
                        z5Edges.append((nodeA,nodeB))
                        z5Edges.append((nodeB,nodeA))
