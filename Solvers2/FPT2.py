...
import numpy as np
from Solvers2.TreeAn import treeToNet, labelTreeSet, getSubSet
...
class Case:
    ...
    def solveCluster(self):
        ...
        labelTreeSet(self.TreeSet)
        childrenDict = nx.get_node_attributes(self.TreeSet[0], 'children')
        root = [n for n in self.TreeSet[0].nodes() if self.TreeSet[0].in_degree(n) == 0]
        childrenDict.pop(root[0])
        for leaf in self.leaves:
            childrenDict.pop(leaf)

        cluster = []
        for node in childrenDict:
            for tree in range(1,len(self.TreeSet)):
                if childrenDict[node] not in nx.get_node_attributes(self.TreeSet[tree], 'children').values():
                    break
                if tree == len(self.TreeSet) - 1:
                    cluster.append(node)
        ...
        for node in cluster:
            if len(childrenDict[node]) > len(childrenDict[biggistNode]):
                biggistNode = node

        subLeaves = childrenDict[biggistNode]
        subSet = getSubSet(self.TreeSet,subLeaves)
