...
import numpy as np
from Solvers2.TreeAn import reLabelTreeSet
...
class Case:
    ...
    def solveCluster(self,k):
        ...
        # showGraph(newTree)
        sol = FPT3_2(newNet,subInTree,input = "net", k = k)
...
def Pick(case):
    ...
    while True:
        ...
        if ripe != None:
            if case.pickLeaf(ripe):
                reduced = True
            #print("Found C ripe ", ripe)
            continue
        if len(case.pickAble) == 1:
            if case.pickAble[0] not in case.getPi2():
                if case.pickLeaf(case.pickAble[0]):
                    reduced = True
                continue
...
def FPT3(T,k = None):
    ...
    #check.append(Case(net, inNrTrees, sIn = []))
    ...
    while len(check) > 0:
        ...
        if len(case.leaves) == 2:
            s = case.s.copy()
            s += [int(case.leaves[0]),int(case.leaves[1])]
        ...
        cher = case.case1()
        if cher != None:
            check.insert(0, Case(copy.deepcopy(case.net), copy.deepcopy(case.inTree), case.leaves.copy(),
                                 case.childK.copy(), copy.deepcopy(case.childD), case.cherries.copy(), case.pickAble.copy(),
                                 case.w, case.C + [(cher[0], cher[1])], case.s.copy()))
            check.insert(0, Case(copy.deepcopy(case.net), copy.deepcopy(case.inTree), case.leaves.copy(),
                                 case.childK.copy(), copy.deepcopy(case.childD), case.cherries.copy(), case.pickAble.copy(),
                                 case.w, case.C + [(cher[1], cher[0])], case.s.copy()))
            #print(case.s)
            #print("cher", cher)
            del case
            continue
        ...
        if triple != None:
            check.insert(0, Case(copy.deepcopy(case.net), copy.deepcopy(case.inTree), case.leaves.copy(),
                                 case.childK.copy(), copy.deepcopy(case.childD), case.cherries.copy(), case.pickAble.copy(),
                                 case.w, case.C + [(triple[1], triple[0])], case.s.copy()))
            check.insert(0, Case(copy.deepcopy(case.net), copy.deepcopy(case.inTree), case.leaves.copy(),
                                 case.childK.copy(), copy.deepcopy(case.childD), case.cherries.copy(), case.pickAble.copy(),
                                 case.w, case.C + [(triple[2], triple[0])], case.s.copy()))
            check.insert(0, Case(copy.deepcopy(case.net), copy.deepcopy(case.inTree), case.leaves.copy(),
                                 case.childK.copy(), copy.deepcopy(case.childD), case.cherries.copy(), case.pickAble.copy(),
                                 case.w, case.C + [(triple[0],triple[1]),(triple[0],triple[2])], case.s.copy()))
            del case
            continue
...
def FPT3_2(T ,subInTree = None, input = 'treeSet',k = None):
    # T: set of trees
    check = []
    sol = []
    ...
    if input == 'treeSet':
        ...
        check.append(Case(T, subInTree, sIn = []))
